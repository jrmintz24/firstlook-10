            ...
            <div className="relative z-10">
              <div className="flex items-center gap-4 md:gap-6 mb-4 md:mb-6">
                ...
                <div>
                  ...
                  <h2 className="text-xl md:text-3xl font-light tracking-tight">{section.title}</h2>
                </div>
              </div>
          ...
          <div className="p-6 md:p-10">
            {isMobile ? (
              <Collapsible open={isContentExpanded} onOpenChange={setIsContentExpanded}>
                ...
                <CollapsibleContent>
                  {/* Key Points with mobile-optimized styling */}
                  <div className="mb-8">
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      {section.content.keyPoints.map((point, pointIndex) => (
                        <div key={pointIndex} className="flex items-start gap-3 p-3 bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl border border-green-200/50">
            ...
            {/* Modern Comparison Table for Introduction Section - Mobile optimized */}
            ...
            {/* Conditional Section CTAs - Mobile optimized */}
            {conditionalCTA && (
              <div className={`p-4 md:p-8 bg-gradient-to-br ${conditionalCTA.gradient.includes('purple') ? 'from-purple-50 to-indigo-50 border-purple-200/50' : conditionalCTA.gradient.includes('green') ? 'from-green-50 to-emerald-50 border-green-200/50' : 'from-blue-50 to-cyan-50 border-blue-200/50'} rounded-xl md:rounded-2xl border`}>
                <div className="flex items-start gap-3 md:gap-4">
                  <div className={`w-10 h-10 md:w-12 md:h-12 bg-gradient-to-br ${conditionalCTA.gradient.split(' hover:')[0]} rounded-lg md:rounded-xl flex items-center justify-center flex-shrink-0`}>
                    <ArrowRight className="w-5 h-5 md:w-6 md:h-6 text-white" />
                  </div>
                  <div className="flex-1">
                    <h4 className="font-medium text-gray-900 mb-2 text-base md:text-lg">{conditionalCTA.title}</h4>
                    <p className="text-gray-600 mb-4 font-light text-sm md:text-base">
                      {conditionalCTA.description}
                    </p>
                    <Link to={conditionalCTA.link}>
                      <Button className={`bg-gradient-to-r ${conditionalCTA.gradient} text-white rounded-lg md:rounded-xl font-medium text-sm md:text-base px-4 md:px-6 py-2 md:py-3`}>
